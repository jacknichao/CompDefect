...
from Models.CompDefect.model import CompDefect as Model
...
class Example(object):
    ...
    def __init__(self, bug_type, fix_func, bug_func, clean_func, index):
...
def read_examples(filename):
    ...
    data = data[['bugType', 'func_code_fix', 'func_code_bug', 'func_code_clean', 'index']]
...
class TextDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        return (torch.tensor(self.examples[item].example_id),
                torch.tensor(self.examples[item].bug_ids),
    ...
    if args.do_train:
        ...
        for epoch in range(args.num_train_epochs):
            ...
            for batch in bar:
                ...
                _, bug_ids, bug_mask, bug_position_ids, bug_attn_mask, clean_ids, clean_mask, clean_position_ids, clean_attn_mask, fix_ids, fix_mask, labels = batch
                loss, cls_loss, gen_loss, _, _, awl_params = model(bug_ids, bug_mask, bug_position_ids, bug_attn_mask,
                                                                   clean_ids,
                                                                   clean_mask,
                ...
                train_cls_loss = round(tr_cls_loss * args.gradient_accumulation_steps / (nb_tr_steps + 1), 4)
                train_gen_loss = round(tr_gen_loss * args.gradient_accumulation_steps / (nb_tr_steps + 1), 4)

                bar.set_description(
                    f'epoch: {epoch} loss: {train_loss} cls_loss: {train_cls_loss} gen_loss: {train_gen_loss} awl_params:{awl_params[0].item()} {awl_params[1].item()}')
            ...
            if args.do_eval and epoch in [int(args.num_train_epochs) * (i + 1) // 20 for i in range(20)] + [
                ...
                for batch in eval_dataloader:
                    ...
                    with torch.no_grad():
                        loss, cls_loss, gen_loss, loss_a, num, _ = model(bug_ids, bug_mask, bug_position_ids,
                                                                         bug_attn_mask, clean_ids,
                                                                         clean_mask,
                                                                         clean_position_ids, clean_attn_mask, fix_ids,
                                                                         fix_mask, labels)
                    eval_loss_a += loss_a.sum().item()
                    ...
                    eval_gen_loss += gen_loss.item()
                # pricing loss of dev dataset
                eval_loss_a = eval_loss_a / tokens_num
                result = {'eval_ppl': round(np.exp(eval_gen_loss), 5),
                          'global_step': global_step + 1,
                          'train_loss': round(train_loss, 5),
                          'train_cls_loss': round(train_cls_loss, 5),
                          'train_gen_loss': round(train_gen_loss, 5),
                          'eval_loss': round(eval_loss, 5),
                          'eval_cls_loss': round(eval_cls_loss, 5),
                          'eval_gen_loss': round(eval_gen_loss, 5),
                          'eval_loss_a': round(eval_loss_a, 5)
                          }
    ...
    if args.do_test:
        ...
        for idx, file in enumerate(files):
            ...
            for batch in tqdm(eval_dataloader, total=len(eval_dataloader)):
                batch = tuple(t.to(device) for t in batch)
                example_ids, bug_ids, bug_mask, bug_position_ids, bug_attn_mask, clean_ids, clean_mask, clean_position_ids, clean_attn_mask, fix_ids, fix_mask, labels = batch
                with torch.no_grad():
                ...
                print(example_ids)
                all_examples_ids.append(example_ids.cpu().numpy())
                for pred in preds:
                    t = pred[0].cpu().numpy()
                    t = list(t)
            ...
            y_trues = np.concatenate(y_trues, 0)
            y_scores = np.concatenate(y_scores, 0)
            all_examples_ids = np.concatenate(all_examples_ids, 0)
            ...
            with open(os.path.join(args.output_dir, f'{os.path.basename(file).split(".")[0]}.output'), 'w') as f, open(
                ...
                os.path.join(args.output_dir, f'{os.path.basename(file).split(".")[0]}.y_score'), 'w') as f4, open(
                os.path.join(args.output_dir, f'{os.path.basename(file).split(".")[0]}.bug'), 'w') as f5, open(
                os.path.join(args.output_dir, f'{os.path.basename(file).split(".")[0]}.example_id'), 'w') as f6:
                print("i'm here")
                for i, (ref, t, t_bug, example_id) in enumerate(zip(p, o, o_bug, all_examples_ids)):
                    # print("REF", ref)
                    ...
                    f5.write(t_bug + '\n')
                    f6.write(str(example_id) + '\n')

                    accs.append(ref == t)
            xmatch = round(np.mean(accs) * 100, 4)
            logging.info("  %s = %s  (%s / %s)" % ("xMatch", str(xmatch), sum(accs), len(accs)))
            logging.info("  " + "*" * 20)
