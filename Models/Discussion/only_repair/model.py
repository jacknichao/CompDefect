...
class Model(nn.Module):
    ...
    def forward(self, bug_ids, bug_mask, bug_position_ids, bug_attn_mask,
                clean_ids=None, clean_mask=None, clean_position_ids=None, clean_attn_mask=None,
        ...
        indices = [i + 1 for i in range(0, bs * 2, 2)]
        indices = torch.tensor(indices).cuda()
        clean_outputs = outputs.index_select(0, indices)
        outputs = bug_outputs
        # outputs = bug_outputs + clean_outputs
        outputs[:, 0, :] = cls_embedding
        # 选出bug_ids的output [l, bsz, hidden]
        ...
        if fix_ids is not None:
            ...
            # logging.info(f'active_loss: {active_loss.sum()} bs: {bs} l: {l}')
            # [bsz, l-[sep], vocab_n]
            ...
            loss = gen_loss
            return loss, None, gen_loss, loss * active_loss.sum(), active_loss.sum(), None
        else:
            # Predict
