...
def main():
        ...
        for epoch in range(args.num_train_epochs):
            ...
            if args.do_eval and epoch in [int(args.num_train_epochs) * (i + 1) // 20 for i in range(20)] + [
                ...
                if eval_loss < best_loss:
                    ...
                    torch.save(model_to_save.state_dict(), output_model_file)
                if eval_loss_a < best_ppl:
                    logging.info("  Best ppl loss:%s", eval_loss_a)
                    logging.info("  " + "*" * 20)
                    best_ppl = eval_loss_a

                    best_loss_output_dir = os.path.join(args.output_dir, 'checkpoint-best-ppl')
                    if not os.path.exists(best_loss_output_dir):
                        os.makedirs(best_loss_output_dir)
                    model_to_save = model.module if hasattr(model, 'module') else model
                    output_model_file = os.path.join(best_loss_output_dir, 'pytorch_model.bin')
    ...
    if args.do_test:
        ...
        for idx, file in enumerate(files):
            ...
            logging.info(f'  Batch size = {args.eval_batch_size}')
            for iiiddd, batch in tqdm(enumerate(eval_dataloader), total=len(eval_dataloader)):
                print(iiiddd, len(eval_dataloader))
                batch = tuple(t.to(device) for t in batch)
                bug_ids, bug_mask, bug_position_ids, bug_attn_mask, clean_ids, clean_mask, clean_position_ids, clean_attn_mask, fix_ids, fix_mask, labels = batch
                with torch.no_grad():
                    _, preds = model(bug_ids, bug_mask, bug_position_ids, bug_attn_mask, clean_ids, clean_mask,
            ...
            with open(os.path.join(args.output_dir, f'{os.path.basename(file).split(".")[0]}.output'), 'w') as f, open(
                ...
                os.path.join(args.output_dir, f'{os.path.basename(file).split(".")[0]}.bug'), 'w') as f2, open(
                os.path.join(args.output_dir, f'{os.path.basename(file).split(".")[0]}.y_trues'), 'w') as f3:
                print("i'm here")
